...
# Import the Operation strategy classes from app.operations.
# Each Calculation subclass delegates its arithmetic to the corresponding Operation class,
# keeping arithmetic logic in one place and the Calculation record separate.
...
class Calculation(ABC):
    ...
    def operand1(self) -> float:
        """Alias for the first operand."""
        return self.a

    @property
    def operand2(self) -> float:
        """Alias for the second operand."""
        return self.b

    @property
    def result(self) -> float:
        """The computed result of this calculation."""
        return self.execute()

    def to_dict(self) -> dict:
        """Serialize this calculation to a plain dictionary."""
        return {
    ...
    def from_dict(cls, data: dict) -> 'Calculation':
        """Deserialize a Calculation from a dictionary produced by to_dict()."""
...
# the `execute` method to perform the specific arithmetic operation by delegating to
# the corresponding Operation strategy class.
...
class AddCalculation(Calculation):
    ...
    **Why Create Separate Classes for Each Operation?**
    ...
    def execute(self) -> float:
        # Delegates to the Addition Operation strategy to perform the addition.
...
class SubtractCalculation(Calculation):
    ...
    **Implementation Note**: This class specifically handles subtraction, keeping
    ...
    def execute(self) -> float:
        # Delegates to the Subtraction Operation strategy to perform the subtraction.
...
class MultiplyCalculation(Calculation):
    ...
    def execute(self) -> float:
        # Delegates to the Multiplication Operation strategy to perform the multiplication.
...
class DivideCalculation(Calculation):
    ...
    **Special Case - Division by Zero**: Division requires extra error handling to
    prevent dividing by zero, which would cause an error in the program. The Division
    Operation strategy validates operands and raises ValidationError for zero divisors.
    """

    def execute(self) -> float:
        # Delegates to the Division Operation strategy, which handles zero-division validation.
...
class PowerCalculation(Calculation):
    ...
    Delegates to the Power Operation strategy, which validates that the exponent
    is non-negative before computing the result.
    """

    def execute(self) -> float:
        # Delegates to the Power Operation strategy to perform exponentiation.
...
class RootCalculation(Calculation):
    """
    RootCalculation represents an nth-root operation.

    Delegates to the Root Operation strategy, which validates that the base is
    non-negative and the root degree is non-zero before computing the result.
    """

    def execute(self) -> float:
        # Delegates to the Root Operation strategy to compute the nth root.
